...
a través de una o varias redes, para comunicar y coordinar sus tareas construyendo
...
compatibilidad entre los dispositivos conectados (la información intercambiada debe entenderse por
...
del marco \texttt{NodIO}). Finalmente,en la Sección \ref{sec:conclusiones} se recogen
...
podemos ver un análisis de algunas plataformas y comparaciones de rendimiento entre ellas. Sin embargo, \texttt{NodIO}
...
\begin{algorithm}[H]
  \caption{Algoritmo utilizado en ``\texttt{NodIO}'' para la realización de experimentos}
  \label{algoritmo}
  \begin{algorithmic}
...
sistema operativo \texttt{GNU/Linux} (distribución \texttt{ArchLinux} con \texttt{kernel} 4.5.1) con
 un procesador \texttt{i5-2410M CPU @2.30GHz}. El navegador utilizado ha sido la
 versión 50 de \texttt{Chromium}, un navegador libre
...
\end{figure}
%
En el experimento se han ido variando el número de pestañas abiertas y se han ejecutado
los algoritmos varias veces (calculando una media ponderada de los tiempos obtenidos para representar la
gráfica - véase la figura \ref{tiempo-pestanas}-).
...
Para ver de forma gráfica los resultados, el lector puede acudir a la figura \ref{tiempo-dispositivos}.
...
de forma más sencilla nuevos experimentos lo que necesite.
