...
\begin{keywords}
  Computaci\'on voluntaria,  algoritmos evolutivos, computaci\'on distribuida,
  computaci\'on en internet, computación nube, evaluaci\'on de rendimiento
\end{keywords}
...
La computación voluntaria es un m\'etodo de c\'omputo mediante el cual se
...
% Enlazar cada sección con la siguiente. Al final de la intro lo que se mete es "El resto del trabajo se organiza de la forma siguiente..." 

\section{Estado del arte}

%Contar qué otros sistemas similares hay en el mercado y qué desventajas pueden tener...

\section{NodIO}

% Nunca cabeceras de secciones vacías. Se hace un pequeño resumen y se introducen las subsecciones
...
% Igual que antes. Aunque lo mejor es no usar sub-subsecciones en un trabajo para conferencia - JJ
...
\begin{algorithm}[H]
  \begin{algorithmic}
    ...
    \State $numero-secuencia \gets 0$
    \State $poblacion-compartida \gets random[]$
    ...
    \ForAll{$clientes$}
       ...
       \Loop
         \State $UUID \gets new-UUID$
         \State $poblacion \gets generarPoblacion()$
         \Repeat
            \While{$poblaciones-generadas\leq n$}
               \State $generarPoblacion()$
            \EndWhile
            \State El $chromosoma$ con mejor $fitness$ es enviado al $servidor$
            \If $Not mejor-global(chromosoma)$
                \State $servidor$ envía un $chromosoma$ aleatorio de $poblacion-compartida$
         ...
         \State El $numero-secuencia \gets numero-secuencia+1$ 
         \State $poblacion-compartida \gets random[]$
         \EndLoop
      \EndFor
...
% Dónde están los datos y el script en R que lo ha generado - JJ
...
% ¿Qué conclusiones se obtienen de los experimentos anteriores?
