...
\title{NodIO: Marco de aplicaciones para computación evolutiva voluntaria}
...
tareas, \texttt{NodIO} no necesita instalar ninguna aplicación ni bibliteca en
...
Diversos son los métodos utilizados para el procesamiento de largos conjuntos de resultados obtenidos
...
de procesamiento (o \textit{cloud}); o, como en el caso que nos ocupa, la computación voluntaria.

En este artículo se desean demostrar las bondades de la computación voluntaria en algoritmos evolutivos,
así como las mejoras de tiempo producidas al utilizar este tipo de computación. Por otra parte, se
introducirá a \texttt{NodIO}, un framework de desarrollo de experimentos
de computación evolutiva en nube, que frente a los \textit{frameworks} actuales,
no precisa de instalación de ningún software por parte del usuario voluntario, salvo un navegador
web.

Los algoritmos genéticos hacen evolucionar una población de individuos -que 
pueden encontrarse codificados de muchas formas- sometiéndolos a 
modifiaciones aleatorias, de forma similar a como ocurre en la evolución biológica
(existiendo operaciones de selección, cruce y mutación entre otras).
...
En estos sistemas se desean características como: tolerancia a fallos -en caso que una o varias de las
máquinas deje de hacer un trabajo, es conveniente que ese cómputo sea realizado por otro usuario y la pérdida 
de datos sea mínima-; ejecución en múltiples máquinas -para obtener la potencia de cálculo necesaria-;
...
La potencia computacional total del sistema variará en función del número de
usuarios que decidan colaborar, así como de la propia potencia de sus máquinas
y la capacidad del nodo ``coordinador" sistema para repartir el trabajo
y recoger los resultados.

La computación voluntaria es importante, entre otras de las cuestiones ya expuestas,
porque permite que ciudadanos fuera del ámbito de la investigación, puedan
tomar parte de la misma.

Además, gracias a que son los usuarios quienes ``prestan" sus máquinas, el sistema
es más barato que la contratación de cómputo o la compra de material dedicado.
...
La computación de algoritmos genéticos puede ser bastante lenta -la velocidad se verá
afectada en función de la longitud de los individuos, la cantidad de estos y las operaciones
que se hagan sobre ellos-, la computación voluntaria es un complemento a la computación de 
escritorio a la hora de obtener más resultados, no teniendo por qué ser algo exclusivo, 
ya que la potencia de los procesadores de hoy día es bastante grande.
...
En cuanto a \textit{frameworks} orientados a computación voluntaria con algoritmos evolutivos, ya en \cite{cole2010evolutionary}
...
Plataforma como Servicio (Platform as a Service). Para el desarrollo de
...
NodIO funciona según un paradigma cliente-servidor. El servidor realiza la coordinación de los clientes, 
...
una interfaz API/REST. Las peticiones (que siguen el ciclo ``CRUD") utilizan el formato JSON. El cliente es 
quien procede con el cómputo y, para obtener y enviar datos, hace peticiones asíncronas al servidor utilizando
la antes mencionada API/REST. En aquellos navegadores en los que esté permitido, se ejecuta utilizando ``Web Workers"
...
menos, dos ficheros: ``is\_solution.js'', 
que se ejecuta en el servidor y decide si el cromosoma enviado por el
cliente es una solución aceptable, y 
``trap.standalone.js'', que se ejecuta en el cliente y es la que
incluye la función de fitness a optimizar y el resto del algoritmo
...
NodIO cuenta con una licencia libre, 
esto no es problema. 
...
solución ejecutar los algoritmos escritos en Javascript en más de una máquina.
...
se ha utilizado el de resolver la función ``trap'', con los valores descritos en la 
tabla \ref{var-table}.
...
móviles LG Bello II con procesador quad-core Cortex-A7 1.3GHz con
Android versión 5.0.2 y navegador Chrome-. 
...
JavaScript es un lenguaje suficientemente rápido como para realizar
